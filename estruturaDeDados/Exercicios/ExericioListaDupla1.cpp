...
// void trocaDados(TElemento *atual, TElemento *substitui, TElemento *aux) {
//     aux->altura = substitui->altura;
//     aux->centroide = substitui->centroide;
//     aux->distancia = substitui->distancia;
//     aux->grupo = substitui->grupo;
//     aux->idade = substitui->idade;
//     aux->imc = substitui->imc;
//     strcpy(aux->nome, substitui->nome);
//     aux->peso = substitui->peso;
    
//     substitui->altura = atual->altura;
//     substitui->centroide = atual->centroide;
//     substitui->distancia = substitui->distancia;
//     substitui->grupo = substitui->grupo;
//     aux->idade = substitui->idade;
//     aux->imc = substitui->imc;
//     strcpy(aux->nome, substitui->nome);
//     aux->peso = substitui->peso;
...
void distribuiElementos(Tlista *L){
	...
	while(atual != NULL){
		if(!atual->centroide){
			//Encontrado um Não-Centróide apontado por atual.
			flag = 0;
			centroide = L->inicio;
			while(centroide != NULL)	{
				//Buscando um centróide para comparação com o não-centróide.
				if(centroide->centroide){
					//Encontrado um centroide...
					if(!flag){
						menor = calcularDistancia(centroide, atual);
						menorDist = centroide;
						flag = 1;
					} else {
						temp = calcularDistancia(centroide, atual);
						if(temp < menor){
							menor = temp;
							menorDist = centroide;
						}//if
				...
				centroide = centroide->prox; //move para o próximo TElemento Centróide.
			}//while
			
			//Já percorreu todos os centróides existentes na Lista...
			atual->grupo = menorDist->grupo;
			
			printf("\n\tCENTROIDE: %s", menorDist->nome);
			printf("\n\tELEMENTO : %s\tGrupo: %d", atual->nome, atual->grupo);
			printf("\n+----------------------------------------------------------+\n");
		}//if
	...
	printf("\n\nEncerrada a Distribuição de Elementos para os Grupos Existentes...\n\n");
}
